...
// ATTENZIONE: niente AdminLayout qui, ci pensa il router con <Outlet/>
export default function AdminLogPage() {
  ...
  useEffect(() => {
    (async () => {
      const r = await adminListLogs(page, level === "all" ? "" : level);
      setRows(((r?.items ?? r?.logs) ?? []) as AdminLogRow[]);
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Select
              value={level}
              onValueChange={(v) =>
                setLevel((v as "all" | "info" | "warn" | "error") ?? "all")
              }
            >
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {rows.map((r, idx) => (
                  <tr key={String(r.id ?? `${page}-${idx}`)} className="border-t">
          ...
          <div className="mt-4 flex items-center justify-between text-sm">
            ...
            <div className="flex gap-2">
              ...
              <button
                className="rounded-md border px-3 py-1 disabled:opacity-50"
                onClick={() => setPage((p) => p + 1)} // <-- parentesi corretta
